...
const PROVISION_SCRIPT_PATH = path.join(__dirname, 'scripts', 'provision-local-user.sh');
...
function normalizeUsername(value) {
  ...
  if (!/^[a-z_][a-z0-9_.-]{0,31}$/.test(username)) {
    throw new Error(
      'Invalid username. Use 1–32 characters: lowercase letters, digits, underscore, dot or hyphen; start with a letter or underscore'
    );
  }
  return username;
}
...
function normalizePublicKey(value) {
  ...
  const supportedType = /^(?:ssh-(?:rsa|ed25519)|ecdsa-sha2-nistp(?:256|384|521)|sk-(?:ssh-ed25519|ecdsa-sha2-nistp256)@openssh\.com)$/;
  if (!supportedType.test(type) || !/^[A-Za-z0-9+/]+={0,2}$/.test(payload)) {
    throw new Error(`Unsupported or malformed SSH public key: ${type}`);
  }
...
function loadProvisionScript(scriptPath = PROVISION_SCRIPT_PATH) {
  let script;
  try {
    script = fs.readFileSync(scriptPath, 'utf8');
  } catch (error) {
    throw new Error(`Cannot read local-user provisioning script ${scriptPath}: ${error.message}`);
  }

  if (!script.startsWith('#!/usr/bin/env bash')) {
    throw new Error(`Invalid provisioning script header: ${scriptPath}`);
  }
  return script;
}
...
function buildProvisionUserScript({
  ...
  replaceAuthorizedKeys = false,
  scriptPath = PROVISION_SCRIPT_PATH
}) {
  const normalizedUsername = normalizeUsername(username);
  const normalizedPassword = normalizePassword(password);
  const normalizedPublicKey = normalizePublicKey(publicKey);
  const script = loadProvisionScript(scriptPath);

  const payload = [
    `XYOPS_LOCAL_USERNAME_B64='${toBase64(normalizedUsername)}'`,
    `XYOPS_LOCAL_PASSWORD_B64='${toBase64(normalizedPassword)}'`,
    `XYOPS_LOCAL_PUBLIC_KEY_B64='${toBase64(normalizedPublicKey)}'`,
    `XYOPS_LOCAL_GRANT_SUDO='${asBoolean(grantSudo, true) ? 'true' : 'false'}'`,
    `XYOPS_LOCAL_PASSWORDLESS_SUDO='${asBoolean(passwordlessSudo, false) ? 'true' : 'false'}'`,
    `XYOPS_LOCAL_REPLACE_AUTHORIZED_KEYS='${asBoolean(replaceAuthorizedKeys, false) ? 'true' : 'false'}'`
  ];
...
function emitProvisionResult({
  ...
  const keyMode = replaceAuthorizedKeys ? 'replace' : 'append';

  emit({
    table: {
      title: 'Local user provisioning result',
      header: ['Host', 'Port', 'User', 'Key mode', 'Sudo', 'Exit code', 'STDOUT', 'STDERR'],
      rows: [[
        host,
        port,
        username,
  ...
  emit({
    data: {
      ssh_action: 'local_user_provision',
      ssh_host: host,
      ssh_port: port,
      provisioned_user: username,
      provisioned_user_sudo: sudoMode,
      provisioned_authorized_keys_mode: keyMode,
...
async function main() {
  ...
  const params = input.params || {};
  const action = required(params.action || 'local_user_provision', 'action');
  if (action !== 'local_user_provision') {
    throw new Error(`Unsupported local-user action: ${action}`);
  }
  ...
  const replaceAuthorizedKeys = asBoolean(params.replace_authorized_keys, false);
  ...
  if (result.code !== 0) {
    emit({
      code: result.code || 1,
      description: `User ${username} provisioning failed on ${host}:${port} with exit code ${result.code}`,
  ...
  emit({ code: 0, description: `User ${username} provisioned on ${host}:${port}` });
}
