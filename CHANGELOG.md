...
- README полностью актуализирован под FreeIPA и SSH-сценарии версии 2.9.0;
- добавлена единая документация по всем Secret Vault переменным и способам SSH-аутентификации;
- описан обязательный порядок импорта Plugin и Workflow файлов;
- во всех пяти Event Plugins расширены понятные `notes` и подсказки полей;
- во всех трёх Workflow Events добавлены подробные описания результата, зависимостей и ограничений;
- обновлены три отдельные инструкции Workflow;
- добавлен regression-тест полноты описаний и документации по секретам.

## v2.8.0

- добавлен отдельный workflow `Создать локального SSH-пользователя на хостах` без FreeIPA;
- workflow использует существующий Plugin `SSH — Создать локального пользователя`;
- поддержаны логин, пароль, открытый ключ, sudo и список SSH-хостов;
- список нод обрабатывается Split Controller, по одному SSH Job на хост;
- существующие workflow FreeIPA и отключения пользователя не изменены.

## v2.7.0

- workflow отключения пользователя больше не запрашивает конкретный публичный ключ;
- SSH-шаг удаляет весь файл `~/.ssh/authorized_keys` указанного локального пользователя;
- Plugin ID `pmlc2ha8fssh_key_remove` сохранён для совместимости импорта;
- локальный пользователь, пароль, домашний каталог и sudo не изменяются;
- отсутствие пользователя, каталога `.ssh` или `authorized_keys` считается успешным no-op;
- активные SSH-сессии не завершаются, вход по паролю отдельно не блокируется.

## v2.6.0

- добавлен workflow `Отключить пользователя FreeIPA → удалить SSH-ключ на хостах`;
- FreeIPA-пользователь отключается через существующий Plugin `Управление FreeIPA`;
- после успешного отключения список SSH-хостов обрабатывается Split Controller;
- добавлен Plugin `SSH — Удалить публичный ключ пользователя`;
- удаляется только указанный ключ из `authorized_keys`, остальные ключи сохраняются;
- локальный пользователь, его пароль, sudo и домашний каталог не удаляются;
- отсутствие пользователя, `authorized_keys` или ключа считается успешным no-op;
- добавлены `ssh-remove-key.js` и `scripts/remove-user-ssh-key.sh`.
...
- из описания параметра SSH-плагина удалено свойство `range`, несовместимое с workflow-макросами при импорте;
...
- поля групп для создания пользователя и изменения членства переведены на multi-select Bucket Menu;
...
- обновлены README, CHANGELOG и `configure-repository.sh`;
- добавлены тесты построения кэша и синхронизации.
